...
USEPATH <- paste(HOME, 'MeltonProteomics', sep='/')
UP1PATH <- HOME
...
pplotDE <- function(lmf, tt, contrast, annot, aspdf) {
    ...
    if (aspdf) {
        pdf(paste(paste(paste(SERVER, 'Pres/Slides_27JAN17', sep='/'), paste('deProteins', contrast, sep='_'), sep='/'), 'pdf', sep='.'))
        #quartz('',5.5, 4.5)
...
pdf(paste(SERVER, 'Pres/Slides_27JAN17/qqp227HousekeepingGenesMarked.pdf', sep='/'))
par.orig <- par(mfrow=c(2,3))
...
plotHousekeeping <- function(hdf, df) {
    ...
    for (spl in spls) {
        ...
        pdf(paste(paste(SERVER, 'Pres/Slides_27JAN17', sep='/'), fn, sep='/'))
...
plotECDF_v1<- function(hdf, df, spl) {
    ld <- stack(df)
...
pdf(paste(SERVER, 'Pres/Slides_27JAN17/venn_DEproteins.pdf', sep='/'))
vennDiagram(res)
dev.off()
...
write.csv(c1Table, file=paste(SERVER, 'Pres/Slides_Dima/c1Table.csv', sep='/'), quote=FALSE)
write.csv(c2Table, file=paste(SERVER, 'Pres/Slides_Dima/c2Table.csv', sep='/'), quote=FALSE)
write.csv(c3Table, file=paste(SERVER, 'Pres/Slides_Dima/c3Table.csv', sep='/'), quote=FALSE)
write.csv(c4Table, file=paste(SERVER, 'Pres/Slides_Dima/c4Table.csv', sep='/'), quote=FALSE)
write.csv(c5Table, file=paste(SERVER, 'Pres/Slides_Dima/c5Table.csv', sep='/'), quote=FALSE)
